...
do ii = 1, np_active
  ...
  !        sngl(P(ii,4)), sngl(P(ii,6)), sngl(P(ii,7)), sngl(P(ii,10))
          if (nind > 0) then
            write(114,23) Time_Next(kk), P(ii,1), P(ii,2), P(ii,3), &
                  P(ii,4), P(ii,5), P(ii,6), P(ii,7), P(ii,8), P(ii,10),  &
                   P(ii,11), P(ii,12), P(ii,13), P(ii,14), P(ii,15), &
                    P(ii,16), P(ii,17), P(ii,18:(nind*2+17))
          else
            write(114,23) Time_Next(kk), P(ii,1), P(ii,2), P(ii,3), &
                  P(ii,4), P(ii,5), P(ii,6), P(ii,7), P(ii,8), P(ii,10),  &
                   P(ii,11), P(ii,12), P(ii,13), P(ii,14), P(ii,15), P(ii,19), P(ii,16), P(ii,17), P(ii,20)

          endif
          npout = npout + 1
          23 FORMAT (F12.5,18(e12.5))

!  !$OMP CRITICAL
  P(ii,:) = P(np_active2,:)
  np_active2 = np_active2 -1

  if (P(ii,8)==1) then!!Mehdi !!this if is for the last cycle of the while
  mean_age = mean_age + P(ii,4)*P(ii,6)!!Mehdi
  mean_comp = mean_comp + P(ii,7)*P(ii,6)!!Mehdi
  total_mass = total_mass + P(ii,6)!!Mehdi
  end if!!Mehdi

end do !Mehdi end of the sorting do while
!  !$OMP END CRITICAL
else
...
! write out summary of mass, age, particles for this timestep
  write(11,'(3(i10),3(f14.5),4(1x,e12.5,1x),3(i8),2(i12))') kk, pfkk, outkk, Time_Next(kk), mean_age , mean_comp, mean_mass, &
...
!!!!!!!MEHDI STORAGE PARTICLE OUTPUT
! Create/open/write the Stroage particles' locations and residence time of each time step
!!! Mehdi If we want have  exited file  for all time steps use filenumEx instead of file name
!open(131,file=trim(runname)//'_StorageParticle.'//trim(adjustl(filenum))//'.txt')
!write(131,*) 'NP X Y Z TIME'
!do ii = 1, np_active
!       write(131,66) ii, P(ii,1), P(ii,2), P(ii,3), P(ii,4), P(ii,5), P(ii,6), P(ii,7), P(ii,8), P(ii,10), P(ii,15), P(ii,19), &
!       PInLoc(ii,1), PInLoc(ii,2), PInLoc(ii,3)
!     66  FORMAT(i10,14(e12.5))
!end do
!flush(131)
!!!!!!!! the above output is produced by mehdi
...
do ii = 1, np_active
        write(13,63) ii, P(ii,1), P(ii,2), P(ii,3), P(ii,4), P(ii,5), P(ii,6), P(ii,7), P(ii,8), P(ii,10), P(ii,15), P(ii,19), &
        PInLoc(ii,1), PInLoc(ii,2), PInLoc(ii,3)
        63  FORMAT(i10,14(e12.5))
end do
!flush(13)
